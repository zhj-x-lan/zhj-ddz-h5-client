cc.Class({
    ...
    onLoad: function () {
        ...
        this._chupaiMode = this.node.getChildByName('chupaiMode');
        for (let i = 0; i < this._chupaiMode.childrenCount; i++) {
            var n = this._chupaiMode.children[i];
            if (n.name == 'toggle') {
                n.setTag(i);
                cc.vv.utils.addToggleEvent(n.getComponent(cc.Toggle), this.node, 'Settings', 'onToggleClicked');
            }
        }
    ...
    onToggleClicked: function (event) {
        cc.vv.gameMgr.chupaiMode = event.target.tag;
    }
});
